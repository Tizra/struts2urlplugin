...
/**
 *
 * Processes strings that contain expressions of the form ${name} and $1, notifying the listener as
 * as each variable is encountered.
 */
public class ExpressionProcessor {
	...
	/**
	 * Substitute values in the context into variables in the input expression. This will substitute
	 * values of the form ${name} and $n where name is the name of a parameter in the MatchContext;
	 * or n is a group number in the MatchContext
	 *
	...
	public String evaluate(String expression) {
		...
		if (expression != null) {
			...
			if (ExpressionProcessor.LOG.isDebugEnabled()) {
				ExpressionProcessor.LOG.debug(expression + " -> " + result.toString());
			}
			return result.toString();
		} else {
			if (ExpressionProcessor.LOG.isDebugEnabled()) {
				ExpressionProcessor.LOG.debug("expression is null");
			}
			return null;
		}
	}
